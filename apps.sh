#!/usr/bin/env bash

#Browsers
flatpak install flathub org.mozilla.firefox -y
flatpak install flathub com.microsoft.Edge -y
flatpak install flathub io.gitlab.librewolf-community -y

#Media
flatpak install flathub com.spotify.Client -y
flatpak install flathub tv.plex.PlexDesktop -y #Client not server
flatpak install flathub org.gnome.Podcasts -y
flatpak install flathub de.haeckerfelix.Shortwave -y

#Locale Media Players
flatpak install flathub org.gnome.Totem -y #for videos
flatpak install flathub com.github.johnfactotum.Foliate -y #for books and comics
flatpak install flathub io.bassi.Amberol -y #for music


#Utilities
flatpak install flathub org.videolan.VLC -y
flatpak install flathub com.github.tchx84.Flatseal -y
flatpak install flathub org.gabmus.whatip -y
flatpak install flathub com.github.unrud.VideoDownloader -y
flatpak install flathub com.raggesilver.BlackBox -y
flatpak install flathub nz.mega.MEGAsync -y
flatpak install flathub org.kde.index -y # file manager

#Coding
flatpak install flathub com.visualstudio.code -y
flatpak install flathub com.axosoft.GitKraken -y
flatpak install flathub org.godotengine.Godot -y



#Social
flatpak install flathub com.gitlab.newsflash -y
flatpak install flathub com.sindresorhus.Caprine -y
flatpak install flathub io.gitlab.caveman250.headlines -y

#Productivity
flatpak install flathub com.todoist.Todoist -y
flatpak install flathub com.github.marktext.marktext -y
flatpak install flathub io.neovim.nvim -y

#Game Launchers
flatpak install flathub com.heroicgameslauncher.hgl -y
flatpak install flathub net.lutris.Lutris -y
flatpak install flathub net.davidotek.pupgui2 -y #protonUp-Qt
flatpak install flathub com.usebottles.bottles -y

#Steamdeck exclusives
flatpak install flathub com.steamdeckrepo.manager -y # Install boot videos to your Steam Deck using Steam Deck Repo website API.
